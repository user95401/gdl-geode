# user95401/gdl-geode

экспериментальный форк мода Geometry Dash Localisation, адаптированный под версию 2.2074 (текущую)
я вырезал много всего, например систему патчей строк, которая требует кучу реверса чтобы портировать вновь и вновь...
вместо неё написаны ужасные (но в целом рабочие) хуки для обновления параметров

<details>
<summary>en</summary>

experimental fork of geometry dash localization mod to run it on 2.2074
i removed a lot of stuff, f.e the string patches system that requires huge reverse engineering work to port again and again
instead of was written awful (but working basicly) params update hooks.
</details>

![image](https://github.com/user-attachments/assets/99ec48e8-08ef-44c2-9454-e9265bb17fe4)

# ИНСТРУКЦИЯ ПО УСТАНОВКЕ:
- зайди в раздел Releases и скачай самую последнюю версию
- установи скачанный .geode файл через кнопку "manual install" в geode loader

<details>
<summary>en</summary>

# INSTALL GUIDE:
- go to releases and download latest one. 
- install downloaded .geode file via "manual install" button in geode loader
</details>

![image](https://github.com/user-attachments/assets/fae3d4d6-81a5-40bb-9d78-2476a8308925)

# GDL

Geometry Dash Localization is a modification that translates Geometry Dash into other languages.

__If you want to translate GD into your language, please message us in our [Discord server](https://discord.gg/CpBA4ehJmX) or @jaan2897 in Discord.__

## Authors
- [__@jaan2897__](https://github.com/JaanDev) - project founder, programmer, website creator, texture and font redrawer.
- [__@myemilia__](https://github.com/mye-creator) - game translator, editor manual translator, creator and owner of the [YouTube channel](https://www.youtube.com/channel/UC7oKo9NgkRz39AAz-6Gb78A).
- [__@demisans__](https://github.com/DemiSans) - game translator, editor guide translator.
- [__@kolyah35__](https://github.com/Kolyah35) - programmer.
- [__@lilray7703__](https://youtube.com/channel/UCK2ADfizaRFJzhei_Qubumg) - texture redrawer.
- [__@pixelsuft__](https://github.com/Pixelsuft) - installer creator (2.1), programmer.
- [__@astrajl___](https://www.youtube.com/channel/UCtC8U-f3iRtm601DcfoUEhg) - texture redrawer.
- [__@maximaldash__](https://www.youtube.com/channel/UCb92kWSQ-5NWIcTZnxHKAfg) - texture redrawer, font designer.
- [__@raelc__](https://github.com/NotRaelc) - game translator.

## Links
- [Their website](https://www.gdlocalisation.uk)
- [Discord server](https://discord.gg/CpBA4ehJmX)
- [Their Github](https://github.com/gdlocalisation)